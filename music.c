...
void on_text(void *data, struct websockets *ws, struct ws_info *info, const char *text, size_t len){
    (void)data; (void)info; (void)ws; (void)len;
...
void on_voice_state_update(
    ...
    query = sqlite3_mprintf("CREATE TABLE IF NOT EXISTS guild_void(guild_id INT, USER_ID INT, voice_channel_id INT);");
...
void on_message(
    ...
    if(0 == strncmp("$play ", msg->content, 6)){
        ...
        while((rc == sqlite3_step(stmt)) != SQLITE_DONE){
            ...
            discord_embed_set_url(&embed, uri->valuestring);
            ...
            ua_reqheader_add(ua, "Client-Name", "MusicBot");
            ua_curl_easy_setopt(ua, NULL, &curl_easy_setopt_cb);

            struct ua_info info1 = {};
            char endpoint[1024];
            ...
            ua_run(ua, &info1, NULL, NULL, HTTP_GET, endpoint);

            struct sized_buffer body = ua_info_get_body(&info1);
...
void *lavalink(void *id){
    ...
    snprintf(url, sizeof(url), "%s", lavalinkNodePassword);
    ...
    ws_reqheader_add(ws, "Client-Name", "MusicBot");
